...
在输入框中输入 `@` 会弹出文件搜索面板，实时搜索 `allowedDirs` 中的文件。输入 `@` 时，最近预览的文件默认显示在列表前方；继续输入关键词后，匹配的最近预览文件仍优先于其他搜索结果。用 ↑↓ 键或鼠标选择文件，回车或点击确认后插入 `[路径]` 格式的引用。
...
文件树固定展示 `knowledge_base/` 与 `inputs/` 两个根目录，并支持最多三级目录浏览。点击文件可在下方预览，支持的文本文件也可进入编辑模式。

根目录列底部会显示最多 3 个最近预览的文件，点击可直接重新打开。
